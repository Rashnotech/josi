...
18. Auth integration: mobile auth uses `JOSI_API_BASE_URL` from `--dart-define`; API URLs are not hardcoded in screens. `AuthRepository` connects login, rider registration, courier registration, forgot password, verify reset code, reset password, logout, and session restore to Laravel `/api/v1/auth/*` endpoints. Tokens are stored with secure token storage and restored through `/api/v1/auth/me`. Validation and network failures stay visible on the current auth screen instead of navigating to a dead screen.
