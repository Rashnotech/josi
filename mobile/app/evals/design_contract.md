...
11. Customer flow: home, select location, manage address, add address, driver details, payment methods, searching rider, active trip, completed trip, trips, profile, notifications, support, and settings are routed. The obsolete customer trip-detail route and screen are not present. The standalone customer wallet screen and customer wallet route are not present. Customer home is map-first, fills the body from the top edge, uses a draggable where-to bottom sheet for destination entry, uses normal mobile text sizing for Current Location, Destination, and Office, sends the Last Trip tile to Activity, uses the history SVG for Activity navigation, uses the office SVG for the Office tile, and lets current-location controls trigger phone GPS. The customer profile menu does not show Notification, Pre-Booked Rides, or Emergency Contact. The customer profile Manage Address item opens the uploaded Manage Address list, not the ride Destination screen, and Add New Address opens the uploaded map-backed Add Address form. The customer Activity screen opens a `Bookings` page with a three-item tab menu for Active, Completed, and Cancelled bookings, compact normal-size text, reference-style driver cards, date/time, route, rating, car number, active map preview actions, and cancelled status labels. Driver names in Activity, ride-found, active-trip, and completed-trip surfaces open the uploaded Driver Details page with About/Review tabs, stats, driver contact, and car details. The shared Settings screen matches the uploaded three-row layout for Notification Settings, Password Manager, and Delete Account. The shared Help Center screen matches the uploaded search layout and uses a two-item tab menu for FAQ and Contact Us, with FAQ category chips and contact cards.
...
17. Rider flow: home, application status, profile setup, bank account details, vehicle setup, available trips, trip request, active trip, completed trip, trips, wallet, cash ledger, notifications, profile, support, and settings are routed. The standalone driver document upload route and screen are not present. The rider application status welcome screen, Continue action, and submitted-verification bottom sheet use normal mobile text and medium action sizing. The bank account details screen collects account number, bank name, and account name only, with no bank-document upload instructions, upload icon area, or attached-file preview.
